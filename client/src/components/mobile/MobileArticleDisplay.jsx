...
/**
 * Enhanced article display component with reader tools
 */
const MobileArticleDisplay = ({
    content,
    title,
    level,
    topic
}) => {
    // Support both 'sections' and 'paragraphs' field names
    const sections = content?.sections || content?.paragraphs || [];
        ...
        return parts.map((part, index) => {
            if (index % 2 === 1) {
                return (
                    <span
                        key={index}
                        className="bg-gradient-to-r from-[#10B981]/30 to-[#059669]/30 text-[#34D399] font-semibold px-1 py-0.5 rounded border-b border-[#10B981]/50"
                    >
                        {part}
                    </span>
                );
    ...
    // Text-to-speech
    const speakSection = (sectionIndex, text) => {
        if ('speechSynthesis' in window) {
            if (isSpeaking && speakingSection === sectionIndex) {
    ...
    return (
        <div className="min-h-screen bg-[#09090B]">
            {/* Compact Header */}
            <div className="px-4 py-3 border-b border-[#27272A] bg-gradient-to-r from-[#10B981]/10 to-transparent">
                <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                            <span className="px-2 py-0.5 bg-[#10B981]/20 rounded-full text-[10px] font-bold text-[#34D399]">
                                {level}
                            </span>
                    ...
                    <button
                        ...
                        className={`p-2 rounded-lg transition-colors ${showTools ? 'bg-[#10B981] text-white' : 'bg-[#27272A] text-[#A1A1AA]'}`}
                    >
                    ...
                    {showTools && (
                        ...
                        >
                            <div className="flex items-center gap-2 pt-3 pb-1">
                                ...
                                <button
                                    onClick={() => {
                                        const allShown = sections.every((_, i) => showTranslation[i]);
                                        const newState = {};
                                        sections.forEach((_, i) => newState[i] = !allShown);
                    ...
                    <div key={index} className="border-b border-[#1F1F23]">
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#10B981] to-[#059669] flex items-center justify-center flex-shrink-0">
                                    <span className="text-white text-xs font-bold">{index + 1}</span>
                                </div>
                                <span className="font-semibold text-white text-left text-[15px]">
                                    {section.heading || section.title || `Paragraph ${index + 1}`}
                                </span>
                            </div>
                            <div className="flex items-center gap-2">
                                {bookmarkedSections[index] && (
                                ...
                                >
                                    <div className="px-4 pb-4 pt-2 bg-[#0A0A0C]">
                                        ...
                                        <div className="flex items-center gap-2 mb-4 pb-3 border-b border-[#1F1F23]">
                                            {/* Read Aloud */}
                                            <button
                                                onClick={() => speakSection(index, section.content)}
                                                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${isSpeaking && speakingSection === index
                                                        ? 'bg-[#EF4444] text-white'
                                                        : 'bg-[#18181B] text-[#A1A1AA] hover:text-white'
                                                    }`}
                                            >
                                                {isSpeaking && speakingSection === index ? (
                                                    <>
                                            ...
                                            <button
                                                ...
                                                className={`p-1.5 rounded-lg transition-colors ${bookmarkedSections[index]
                ...
                <div className="fixed bottom-20 left-0 right-0 px-4 py-3 bg-[#18181B]/95 backdrop-blur border-t border-[#27272A]">
                    <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
                        ...
                        {content.vocabulary.map((word, i) => (
                            <span
                                key={i}
                                className="px-2.5 py-1 bg-[#27272A] rounded-full text-xs text-[#34D399] font-medium whitespace-nowrap"
                            >
                                {word}
                            </span>
                        ))}
...
export default MobileArticleDisplay;
